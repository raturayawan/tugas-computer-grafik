 ...
 * @author ASUS
 */
public class Raturayawan extends Application implements Runnable {
    ...
    private void update() {
     if (inputKeyboard.contains("RIGHT")){//Menggerakkan kotak ke kanan saat key RIGHT ditekan
         cx++;
     }else if (inputKeyboard.contains("LEFT")){//Menggerakkan otak ke kiri saat key LEFT ditekan
         cx--;
     } 
    
     if(inputKeyboard.contains("UP")){//Menggerakkan kotak keatas saat key UP ditekan
         cy--;
     }else if(inputKeyboard.contains("DOWN")){//Menggerakkan kotak kebawa saat key DOWN ditekan
         cy++;
         
     }
     if(inputKeyboard.contains("R")){//Rotasi kotak keara gerakan jarum jam saat ke R ditekan
         sudutrotasi++;
     }
    }
    ...
    private void draw() {
        try {
            if (canvas != null) {
                ...
                //CONTOH MENGGAMBAR KOTAK DAPAT DITRANSAKSI DAN DIROTASI
                gc.save();
                gc.translate (cx, cy);
